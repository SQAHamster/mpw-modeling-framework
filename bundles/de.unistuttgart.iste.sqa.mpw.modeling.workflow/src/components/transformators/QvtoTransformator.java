 ...
 * 
 * It uses modelslots to obtain the input models. Optionally a target slot can
 * be specified where the result will be written to.
 */
public class QvtoTransformator extends WorkflowComponentWithModelSlot {
    ...
    @Override
    protected void invokeInternal(final WorkflowContext workflowContext, final ProgressMonitor monitor,
            final Issues issues) {
        executor = new ModelToModelTransformationExecutor(workflowContext, issues, transformationUri,
        ...
        new LambdaVisitor<Object>().on(List.class).then(modelInstances -> {
            transformList(modelInstances);
        }).on(EObject.class).then(eObject -> {
            transformSingleObject(eObject);
        }).orElse(() -> {
            throw new IllegalArgumentException("can only accept list of EObjects or single EObject instances");
        }).accept(slotContent);

        log.info("Transformed " + executor.getSuccessfulTransformationsCount() + " models successfully with "
                + executor.getTransformationName());
    ...
    private void addResultInstancesToSlot(final WorkflowContext workflowContext) {
        ...
        if (previousSlotContent != null && mergeToExistingSlotContent) {
            new LambdaVisitor<Object>().on(List.class).then(modelInstances -> {
                modelInstances.addAll(resultInstances);
            }).orElse(() -> {
