...
/**
 * Helper class used to retain annotations, which are stored in .ecore files,
 * but not available when using generated Ecore packages.
 *
 * Main usecase: Attach documentation annotations from a loaded, but not
 * registered Ecore model to the registered one.
 */
public class AnnotationRetainer {
    ...
    private static void traverseAllModelElements(final EPackage ePackage, final Consumer<EModelElement> consumer) {
        ...
        while (contentIterator.hasNext()) {
            ...
            new LambdaVisitor<Object>().on(EModelElement.class).then(element -> consumer.accept(element))
                    .accept(content);
        }
    }
    ...
    private static Optional<String> toKey(final EModelElement content) {
        ...
        new LambdaVisitor<EModelElement>().on(EClassifier.class)
                .then(eClassifier -> key.append(getClassifier(eClassifier))).on(EStructuralFeature.class)
                .then(eFeature -> key.append(getFeature(eFeature))).on(EOperation.class)
                .then(eOperation -> key.append(getOperation(eOperation))).accept(content);
