...
public class EclipsePathHelper {
	...
	/**
	 * Checks for a project, if it is located in the current workspace.
	 * Background: Some resource access is done in the same workspace, which needs intermediary source directories (e.g. "myproject/src/myresource.txt").
	 * But, this might also be the case in external projects, where the project is included by a dependency archive.
	 * This needs to remove the source directory part (e.g. "myproject/myresource.txt"), since it is placed inside the JAR archive.
	 * 
